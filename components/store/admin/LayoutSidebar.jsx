...
export default function StoreLayoutSidebar({store, title}) {
    ...
    const [ storeSlide, setStoreSlide ] = useState(title === "Plantillas" ? true : false);

    function handleStoreSlide() {
        setTimeout(() => {
            setStoreSlide(true);
        }, 100)
    }
        ...
        <>
            ...
            <div className={`${!active && 'hidden'} shadow-sm flex lg:hidden cursor-pointer items-center justify-center rounded-full absolute top-4 right-5 transition-colors z-10 text-xl`} onClick={handleClose}><i className="fa-regular fa-arrow-left-from-line"></i></div>
                <div className={`relative flex flex-col gap-1 py-5 pt-14 lg:pt-5 h-full w-full`}>
                    ...
                    <Link href={`/store/${store}/admin/orders`}>
                    ...
                    <Link href={`/store/${store}/admin/products`}>
                    ...
                    <Link href={`/store/${store}/admin/clients`}>
                    ...
                    <Link href={`/store/${store}/admin/discounts`}>
                    ...
                    <div>
                        <div className="ml-5 text-sm mt-5">Mi Tienda</div>
                        <Link href={`/store/${store}/admin/templates`}>
                            <div className={`border-transparent border-l-4 pl-4 p-2 mr-5 rounded-tr-md rounded-br-md hover:bg-zinc-100 transition-colors`} onClick={handleStoreSlide}>
                                <div className="flex gap-2 items-center">
                                    <i className={`${title == "Temas" || storeSlide ? "fa-solid text-main-color" : "fa-regular"} fa-store text-lg w-6 text-center`}></i>
                                    <div>Mi Tienda</div>
                                </div>
                            </div>
                        </Link>
                        { storeSlide && (
                            <div>
                                <Link href={`/store/${store}/admin/templates`}>
                                    <div className={`${title == "Plantillas" ? "bg-zinc-100 border-main-color" : "border-transparent"} border-l-4 pl-4 p-2 mr-5 rounded-tr-md rounded-br-md hover:bg-zinc-100 transition-colors`}>
                                        <div className="flex gap-2 items-center">
                    ...
                    <div className="absolute bottom-0 bg-white w-full pr-5">
                        <Link href={`/store/${store}/admin/settings`}>
